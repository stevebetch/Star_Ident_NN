...
/*
 ...
 typedef Eigen::Matrix<int, Eigen::Dynamic, 1> VectorXd;
 ...
 class NeuNet {
 public:
 NeuNet(int nodeIn, int NodeOut, int NodeHidden, long double initWmin, long double initWmax, long double eta, long double mo);
 //default constuctor. creates a neural net with random values
 
 long double Upd(long double Upvals[]);
 // Modifies: none
 // effect: propigates the data thru the neural net
 
 long double BP(long double TargetVals[]);
 //back propigates the error thru the NN.
 
 long double NNout(string fname);
 
 int loadNN(string Inname);
 
 };
 
 long double training(NeuNet NN, long double dec1, long double dec2, long double ra1, long double ra2, long double limit );
 
 long double tester(NeuNet NN, int star);
...
/*
    ...
    for (int i=0; i<(NN.NodeIn.size()); i++) {
        for (int j=0; j<(NN.nodeHidden.size()); i++) {
            //printf("%d %ld\n",i,(NN.NodeIn.size()-1));
...
long double NeuNet::BP(selfNN &NN, double TargetVals){
    /*
     #driven by mitchell, pg 98, Backpropagation algorithm
     
     # error management here..... dont wanna crash and not know why....
     
     #if range(TargetValues)!= self.NodeOut:
     #       raise UhohError, ' Shit just hit the fan cuz the wrong number of values were passed to BP!'
     
     
     #Starting at the end, evaluate deltk for all the output units using (5.54, (bishop, p 244))
     
     porting to C++ for speed.....
     */
   // cout<<"\ndelta out:";
...
long double training(NeuNet &starz, long double dec1, long double dec2, long double ra1, long double ra2, long double limit, selfNN &NN ){
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////
    moo=0; // double check.
    VectorXd dist(NN.nodeOut.size());
     printf("\nStarting. Num stars:%Lf \n",retval);
    while(moo<limit)
    {
        moo+=1;
      //  printf("\nitteration:%d \n",moo);
